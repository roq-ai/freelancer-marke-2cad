...
import { useRoqClient, useInvoiceFindManyWithCount } from 'lib/roq';
...
export function InvoiceListPage(props: InvoiceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useInvoiceFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.invoice.delete({ where: { id } });
      await mutate();
    } catch (error) {
